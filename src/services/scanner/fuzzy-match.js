...
let dictionaryNormalized = null; // Lowercase with punctuation removed for fuzzy matching
...
function ensureDictionary() {
    ...
    dictionaryNormalized = cardNames.map(n => n.toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim());
...
function findBestMatch(text, maxDistance = LOOKUP_DISTANCE) {
    ...
    if (dictionarySet.has(lower)) {
        return { name: dictionary[dictionaryLower.indexOf(lower)], distance: 0 };
    }

    const normalizedText = lower.replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim();
    let bestName = null;
    let bestDist = maxDistance + 1;

    for (let i = 0; i < dictionaryNormalized.length; i++) {
        const dist = levenshtein(normalizedText, dictionaryNormalized[i], bestDist);
        if (dist < bestDist) {
            bestDist = dist;
            bestName = dictionary[i]; // Return the properly-cased name
            if (dist === 0) break; // Exact match on normalized text, stop searching
        }
    }
