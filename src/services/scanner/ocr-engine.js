...
function parseHocr(hocrHtml) {
    ...
    while ((match = lineRegex.exec(hocrHtml)) !== null) {
        ...
        // Extract confidence from words inside the line (ocr_line doesn't have x_wconf)
        let totalConf = 0;
        let wordCount = 0;
        const wordConfRegex = /x_wconf\s+(\d+)/g;
        let wordMatch;
        while ((wordMatch = wordConfRegex.exec(innerHtml)) !== null) {
            totalConf += parseInt(wordMatch[1], 10);
            wordCount++;
        }
        const confidence = wordCount > 0 ? Math.round(totalConf / wordCount) : 50;
